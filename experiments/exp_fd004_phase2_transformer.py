"""
FD004 Phase-2 Transformer Experiment

This script runs a Phase-2 experiment for FD004 with:
- Condition embeddings (7 conditions for FD004)
- HI smoothness loss (optional)
- Transformer encoder (instead of LSTM)

Usage:
    python -m experiments.exp_fd004_phase2_transformer
"""
...
def main():
    print("=" * 80)
    print("FD004 Phase-2 Transformer Experiment")
    print("=" * 80)
    print("\nPhase-2 Features:")
    ...
    # ===================================================================
    # Configuration (Phase-2 Transformer)
    # ===================================================================
    dataset_name = "FD004"
    ...
    # Encoder configuration - Transformer
    encoder_type = "transformer"  # Use Transformer instead of LSTM
    # Note: hidden_dim must be divisible by transformer_nhead
    # For nhead=4, hidden_dim should be 48, 52, 56, etc. (not 50)
    hidden_dim = 48  # Adjusted to be divisible by nhead=4
    num_layers = 2
    dropout = 0.1
    bidirectional = False  # Not applicable for Transformer
    transformer_nhead = 4  # Number of attention heads
    transformer_dim_feedforward = 256  # Feedforward dimension
    ...
    # Validate hidden_dim is divisible by nhead
    if hidden_dim % transformer_nhead != 0:
        raise ValueError(
            f"hidden_dim ({hidden_dim}) must be divisible by transformer_nhead ({transformer_nhead}). "
            f"Please adjust hidden_dim (e.g., {transformer_nhead * (hidden_dim // transformer_nhead)}) or nhead."
        )
    ...
    model = EOLFullLSTMWithHealth(
        ...
        # Phase 2: Encoder type - Transformer
    ...
    print(f"  - Transformer config: d_model={hidden_dim}, nhead={transformer_nhead}, "
    ...
    print("\n" + "=" * 80)
    print("Phase-2 Transformer Experiment Complete")
    print("=" * 80)
    print(f"\nTest Metrics:")
